...
- Windows 与 Android 同一 Wi-Fi 主动加密双向同步，传输完成后立即关闭临时入口；
- 桌面、平板和手机响应式布局。
...
以下截图来自 `v0.3.2` 本地真实运行页面（`1440×900`），不是设计稿或合成图。

| 今日词汇任务与 AI 助理 | 单词学习工作台 |
| --- | --- |
| ![首页：今日词汇任务、AI 助理与当前段位](docs/images/home-desktop.png) | ![单词学习：队列、语境题面与学习进度](docs/images/study-desktop.png) |
| **精读训练与文章标注** | **科举十级段位档案** |
| ![阅读训练：文章目录、正文、字号与标注工具](docs/images/reading-desktop.png) | ![成长页面：科举十级段位与六维学习证据](docs/images/growth-desktop.png) |
...
Windows 与 Android 安装版可在“我的 → 设备同步”中主动合并数据。同步使用临时 TLS 证书、一次性令牌和同步前数据库备份，不经过云端；DeepSeek API Key、会话密钥和设备权限不会进入同步包。
...
- AI 写入笔记或记忆前需要用户确认；
- 局域网同步主服务仍只监听本机地址，临时同步端口最长开放 10 分钟；
