...
def fetch(article):
    ...
    source_text = html.unescape(re.sub(r"(?s)<[^>]+>", " ", raw))
    source_words = TOKEN.findall(source_text.lower())
...
path = root / "seed" / "review_batches" / "technology_l1_l3_manual.json"
articles = json.loads(path.read_text(encoding="utf-8-sig"))["passages"]
results, failures = {}, {}
...
if failures or any(results.values()):
    raise SystemExit(1)
